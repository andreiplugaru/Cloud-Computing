<script lang="ts">
	import { authenticate } from '../../stores/user';
	...
	async function login(event: Event) {
		...
		const response = await authenticate(data.get("email") as string);
		if (response.ok) {
			error = false;
			await goto("/home");
		...
		<div class="w-full bg-white rounded-lg shadow-2xl dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form class="space-y-4 md:space-y-6" on:submit|preventDefault={login}>
					...
					<p class="text-error-700" class:hidden = {!showError}>The email doesn't exist</p>
