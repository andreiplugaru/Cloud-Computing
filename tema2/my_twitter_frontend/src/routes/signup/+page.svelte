<script lang="ts">
	import { createAccount } from '../../stores/user';
	...
	async function signup(event: Event) {
		...
		const response = await createAccount(data.get("email") as string, data.get("name") as string);
		if (response.ok) {
			error = false;
			await goto("/home");
...
<section class="bg-gray-50 dark:bg-gray-900 h-0 " style="position: absolute;
  /* new */
  left: 50%;
			...
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form class="space-y-4 md:space-y-6" on:submit|preventDefault={signup}>
					...
					<div>
						...
						<input name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name" required="">
					</div>
					...
					<p class="text-error-700" class:hidden = {!showError}>{messageError}</p>
